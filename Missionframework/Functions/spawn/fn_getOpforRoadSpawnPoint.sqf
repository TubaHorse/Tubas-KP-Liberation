/*
    ...
    Description:
        Gets a random opfor road spawn point marker name respecting following conditions:
...
private ["_valid", "_current", "_distances"];
{
    ...
    // Check for not activated sectors
    if (_valid) then {
        if ((KPLIB_sectors_all - KPLIB_sectors_active) findIf {((markerPos _current) distance2D (markerPos _x)) < 1000} < 0) then {
            _valid = false;
        };
    };
