...
private _entities = (_pos nearEntities ["CAManBase", _radius]) select {
    (side (group _x) == _side) 
    && !(captive _x) && ((getpos _x) select 2 < 500) 
    && lifeState _x != "INCAPACITATED"
};

{
    _entities pushBack _x;
} forEach ((_pos nearEntities [["Car", "Tank", "Air", "Ship"], _radius]) select {
    ((getpos _x) select 2 < 500) 
    && (count (crew _x) > 0) 
    && (({(side (group _x) == _side)} count (crew _x)) > 0)
});
