...
if (KPLIB_param_fullHealCheckEnemies && ([_centerPos, 350, KPLIB_side_enemy ] call KPLIB_fnc_getUnitsCount) > 1 ) exitWith {
    [localize "STR_FULLHEAL_ENEMIES_NEARBY", true, 3] remoteExecCall ["KPLIB_fnc_hint", _caller];
};
...

{
    ...
    if (_cooltime < _localtime) then {
        ...
        if (_target isEqualTo _caller) then {
            [format [localize "STR_FULLHEAL_COOLDOWN_OWN", round _cooldowntime], true, 3] remoteExecCall ["KPLIB_fnc_hint", _target];
        } else {
            [format [localize "STR_FULLHEAL_COOLDOWN", round _cooldowntime, name _caller], true, 3] remoteExecCall ["KPLIB_fnc_hint", _target];
        };
    };
} forEach _targetunits;

if (!(_caller in _cooldownunits)) then {
    [localize "STR_FULLHEAL_DONE", false, 5] remoteExecCall ["KPLIB_fnc_hint", _caller];
};
[format [localize "STR_FULLHEAL_APPLY", name _caller], false, 5] remoteExecCall ["KPLIB_fnc_hint", _healedunits];
