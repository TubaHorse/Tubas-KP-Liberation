...
if (_sector in KPLIB_sectors_airport) then {
	// Create an area bigger than the area itself to be used as whitelist
	private _mineArea = createMarkerLocal [format ["mineArea_%1", _sector], markerPos _sector];
	_mineArea setMarkerShapeLocal (markerShape _sector);
	_mineArea setMarkerSizeLocal [((markerSize _sector) # 0)*1.4, ((markerSize _sector) # 1)*1.4];
	_mineArea setMarkerBrushLocal "Border";
	_mineArea setMarkerDir (markerDir _sector);

	private _lastPos = markerPos _sector;
	for "_i" from 1 to 12 do {
		private _pos = [[_mineArea], [], {
			//((_this nearEntities [["LandVehicle"], 10]) isEqualTo []) 
			(_this isFlatEmpty [-1, -1, 1, 10, 0] isNotEqualTo [])
			&& !(_this inArea _sector) // Not inside airport area
	...
	for "_i" from 1 to 4 do {
		private _pos = [[_mineArea], [], {
			((isOnRoad _this) || (_this isFlatEmpty [5, -1, 1, 10, 0] isNotEqualTo [])) 
			//&& ((_this nearEntities [["LandVehicle"], 10]) isEqualTo [])  
			&& !(_this inArea _sector) // Not inside airport area
