...
//if (KPLIB_enemyReadiness <= (80 - (5 * KPLIB_param_aggressivity))) then {_artillery_pool = KPLIB_o_artilleryHeavy} else {_artillery_pool = KPLIB_o_artilleryLight}; // placeholder

_selected_opfor_artillery = selectRandom _artillery_pool;

// Get arty class
private _artyClass = _selected_opfor_artillery select 0;
...
if (isNil "KPLIB_artilleryPosition_Down") then {
	// Create this to count each time an enemy artillery position is eliminated per session to punish enemies (by adding delay to the next spawn)
	KPLIB_artilleryPosition_Down = 0;
};
...
if !(_spawn_marker isEqualTo "") then {
	...
	if (KPLIB_enemyReadiness >= 40) then {_repeatSpawn = 2};
	if (KPLIB_enemyReadiness >= 75) then {_repeatSpawn = 3};
...
[format["Artillery spawned in %1, near %2", markerPos _spawn_marker, [getMarkerPos _spawn_marker] call KPLIB_fnc_getLocationName], "ARTILLERY POSITION"] call KPLIB_fnc_log;
