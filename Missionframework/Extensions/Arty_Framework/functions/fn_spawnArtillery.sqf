/* 
	...
	Parameter(s):
		_pos - position where the artillery will spawn [POSITION, defaults to [0,0,0]]
		_classname - classname of the artillery [STRING, defaults to ""]
...
// LAMBS Artillery Called
["lambs_danger_OnArtilleryCalled", {
	params ["_caller","_groupCaller","_gun","_targetPos"];
	_playersInArea = allPlayers select {_x distance _targetPos < 250};
	if (count _playersInArea == 0) exitWith {};
	[] remoteExec ["remote_call_newartillery_firing", _playersInArea];
