scriptName "remote_call_fob";

if ( isDedicated ) exitWith {};

if ( isNil "sector_timer" ) then { sector_timer = 0 };

params [ "_fob", "_status" ];

private _fobname = [_fob] call KPLIB_fnc_getBaseName;

if (_fob in KPLIB_player_outposts) then {
    // Outpost notifications
    if ( _status == 0 ) then {
        [ "lib_outpost_built", [ _fobname ] ] call BIS_fnc_showNotification;
    };

    if ( _status == 1 ) then {
        [ "lib_outpost_attacked", [ _fobname ] ] call BIS_fnc_showNotification;
        "opfor_capture_marker" setMarkerPosLocal _fob;
        sector_timer = KPLIB_vulnerability_timer;
    };

    if ( _status == 2 ) then {
        [ "lib_outpost_lost", [ _fobname ] ] call BIS_fnc_showNotification;
        "opfor_capture_marker" setMarkerPosLocal markers_reset;
        sector_timer = 0;
        KPLIB_player_outposts = KPLIB_player_outposts deleteAt (KPLIB_player_outposts find _fob)
    };

    if ( _status == 3 ) then {
        [ "lib_outpost_safe", [ _fobname ] ] call BIS_fnc_showNotification;
        "opfor_capture_marker" setMarkerPosLocal markers_reset;
        sector_timer = 0;
    };
} else {
    // Fob notifications
    if ( _status == 0 ) then {
        [ "lib_fob_built", [ _fobname ] ] call BIS_fnc_showNotification;
    };

    if ( _status == 1 ) then {
        [ "lib_fob_attacked", [ _fobname ] ] call BIS_fnc_showNotification;
        "opfor_capture_marker" setMarkerPosLocal _fob;
        sector_timer = KPLIB_vulnerability_timer;
    };

    if ( _status == 2 ) then {
        [ "lib_fob_lost", [ _fobname ] ] call BIS_fnc_showNotification;
        "opfor_capture_marker" setMarkerPosLocal markers_reset;
        sector_timer = 0;
    };

    if ( _status == 3 ) then {
        [ "lib_fob_safe", [ _fobname ] ] call BIS_fnc_showNotification;
        "opfor_capture_marker" setMarkerPosLocal markers_reset;
        sector_timer = 0;
    };
};





